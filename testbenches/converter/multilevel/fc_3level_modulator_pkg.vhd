----------------------------------
-- Shared bits of the 3-level flying-capacitor modulation scheme, used by
-- both fc_3level_modulator (the entity) and fc_3level_tb (which models the
-- bridge). Mirrors fc_4level_modulator_pkg for the 2-cell stack.
----------------------------------
LIBRARY ieee  ;
    use ieee.math_real.all;

package fc_3level_modulator_pkg is

    -- gate pattern of the 2 flying-cap cells, "ab"
    subtype sw_states is bit_vector(1 downto 0);

    -- switch-state sequence : one row per output level (0..1), one column
    -- per state of the 4-state sequence
    type sw_vector     is array (natural range <>) of bit_vector;
    type sw_seq_matrix is array (natural range <>) of sw_vector;
    constant fc_3_sw_matrix : sw_seq_matrix(0 to 1)(0 to 3)(0 to 1) := (
        0 => ("01", "00", "10", "00"),
        1 => ("11", "01", "11", "10"));

    -- one output level's row of the sequence : its 4 gate patterns
    subtype sw_sequence    is sw_vector(0 to 3)(0 to 1);
    -- one value per state of the 4-state sequence
    subtype sw_time_vector is real_vector(0 to 3);
    ...
    type fc_modulator_output_record is record
        ...
        level_switch_vector : sw_sequence;       -- the active level's 4 gate patterns
    end record;
    ...
    constant fc_modulator_output_init : fc_modulator_output_record := (
        ...
        next_switch_pattern => "01",
        switching_time      => 0.0,
        switching_times     => (others => 0.0),
        level_switch_vector => fc_3_sw_matrix(0));
    ...
    -- flying-cap balancing : one dwell-time trim per state of the 4-state
...
end package fc_3level_modulator_pkg;

package body fc_3level_modulator_pkg is
    ...
    function get_fc_trims(
        ...
        variable trim   : real;
        variable retval : real_vector(0 to 3) := (others => 0.0);
    begin
        for s in 0 to 3 loop
            pat := fc_3_sw_matrix(level)(s);
...
end package body fc_3level_modulator_pkg;
