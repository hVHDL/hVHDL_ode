----------------------------------
-- 3-level flying-capacitor modulator.
--
-- Same shape as fc_4level_modulator / fc_5level_modulator, for the 2-cell
-- stack. Its signals are wrapped into records (fc_modulator_input_record /
-- fc_modulator_output_record, in fc_3level_modulator_pkg); clock is a
...
--                                            the output level (row of fc_3_sw_matrix);
...
--                                            4-state sequence. Supply flying-cap
...
--   out  modulator_out.next_switch_pattern - "ab" gate pattern for the coming sub-interval
...
--   out  modulator_out.level_switch_vector - the active level's 4 gate patterns
--
-- Each request emits one state of the 4-state sequence : its switch pattern
-- from fc_3_sw_matrix and a dwell of the nominal level time
...
begin
    ...
    modulator : process(clock)
        variable state_index : natural range 0 to 3 := 0;
        variable d           : real := 0.0;
        variable level       : natural range 0 to 1 := 0;
    ...
    begin
        if rising_edge(clock) then
            ...
            if modulator_in.modulation_requested then
                ...
                modulator_out.next_switch_pattern <= fc_3_sw_matrix(level)(state_index);
                modulator_out.switching_time      <= times(state_index);
                modulator_out.switching_times     <= times;
                modulator_out.level_switch_vector <= fc_3_sw_matrix(level);
