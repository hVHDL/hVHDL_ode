----------------------------------
-- 3-level flying-capacitor converter, switching model.
--
-- Same structure as fc_4level_tb : a stimulus process, a reference process
-- and a modulator entity, joined by the sim_ready / modulation_ready
-- handshake :
...
--   fc_3level_modulator - takes that duty_ratio and, on each request, emits
--                          the next switch pattern and the time to apply it.
--
-- Open loop : no flying-cap balancing (switching_time_trim left at 0). The
-- dither on the modulator reference broadens its spectrum so the
...
entity fc_3level_tb is
  generic (runner_cfg : string);
end;

architecture vunit_simulation of fc_3level_tb is
    ...
    signal modulator_reference : real := initial_dc_link/4.0;  -- command / logging
    signal duty_ratio          : real := 0.25;
    -- stimulus -> p_reference
    signal il_meas             : real := 0.0;                  -- inductor current feedback

    ----------------------
    -- bridge voltage of the 2-cell stack for a gate pattern (plant side)
...
------------------------------------------------------------------------
    ...
    stimulus : process(simulator_clock)
        ...
        constant init_state_vector : real_vector :=
            (0 => 0.0, 1 => initial_dc_link/2.0, 2 => initial_dc_link/2.0);
        ...
        begin
            bridge_voltage := get_fc_bridge_voltage(mod_sw_state, udc_sig, (0 => ufc1));
        ...
        file file_handler : text open write_mode is "fc_3level_tb.dat";
    ...
    begin
        if rising_edge(simulator_clock) then
            ...
            if not simulation_started then
                write_plot_config(file_handler, "title", "3-level flying-capacitor converter");
                write_plot_config(file_handler, "T_title", "Inductor current");
                write_plot_config(file_handler, "T_ylabel", "Current [A]");
                write_plot_config(file_handler, "B_title", "Output, flying-cap and DC-link voltages");
                ...
                write_plot_config(file_handler, "label_B_u1", "Flying-cap voltage");
                write_plot_config(file_handler, "label_B_u2", "DC-link voltage");
                write_plot_config(file_handler, "label_B_u3", "Modulator reference");
                ...
                write_plot_config(file_handler, "freq_xlim", "1000,200000");
                ...
                write_plot_config(file_handler, "freq_pair_uC", "B_u3,B_u0");
                write_plot_config(file_handler, "freq_pair_iL", "B_u3,T_i0");
                ...
                write_to(file_handler,(realtime
                        ...
                        , lcr_rk5(2)          -- B_u1 : flying-cap voltage
                        , udc_sig             -- B_u2 : DC link
                        , modulator_reference -- B_u3 : modulator reference
                    ));
...
------------------------------------------------------------------------
    p_reference : process(simulator_clock)
        ...
        constant voltage_offset : real := initial_dc_link/4.0;

        variable udc : real := initial_dc_link;
        variable seed1, seed2 : positive := 1;
        variable rand : real;
        variable reference : real := initial_dc_link/4.0;
    begin
        if rising_edge(simulator_clock) then
            ...
            if sim_ready then
                ...
                if realtime > 150.0e-3 then reference := 100.0; end if;
                if realtime > 250.0e-3 then reference := 150.0; end if;
...
------------------------------------------------------------------------
    -- fc_3level_modulator : one 0..1 duty over the whole 0..Udc output range,
    -- no flying-cap balancing (switching_time_trim left at 0)
    ...
    u_modulator : entity ode.fc_3level_modulator
        port map (
