----------------------------------
-- Shared bits of the 5-level flying-capacitor modulation scheme, used by
-- both fc_5level_modulator (the entity) and fc_5level_tb (which builds the
-- balancing trim vector and models the bridge).
----------------------------------
LIBRARY ieee  ;
    use ieee.math_real.all;

package fc_5level_modulator_pkg is

    -- gate pattern of the 4 flying-cap cells, "abcd"
    subtype sw_states is bit_vector(3 downto 0);

    -- switch-state sequence : one row per output level (0..3), one column
    -- per state of the 8-state sequence
    ...
    -- one output level's row of the sequence : its 8 gate patterns
    subtype sw_sequence    is sw_vector(0 to 7)(0 to 3);
    -- one value per state of the 8-state sequence
    subtype sw_time_vector is real_vector(0 to 7);
    ...
    type fc_modulator_output_record is record
        ...
        level_switch_vector : sw_sequence;       -- the active level's 8 gate patterns
    end record;
    ...
    constant fc_modulator_output_init : fc_modulator_output_record := (
        ...
        next_switch_pattern => "0001",
        switching_time      => 0.0,
        switching_times     => (others => 0.0),
        level_switch_vector => fc_5_sw_matrix(0));
    ...
    -- flying-cap balancing : one dwell-time trim per state of the 8-state
...
end package fc_5level_modulator_pkg;

package body fc_5level_modulator_pkg is
    ...
    begin
        ...
        r := integer(floor(d * 4.0));
        if r > 3 then r := 3; end if;
        return r;
    end to_level;
    ...
    function get_fc_trims(
        ...
        variable trim   : real;
        variable retval : real_vector(0 to 7) := (others => 0.0);
    begin
        for s in 0 to 7 loop
            pat := fc_5_sw_matrix(level)(s);
...
end package body fc_5level_modulator_pkg;
